...
import { LegacyNFT, Transfer } from '../../entities/LegacyNFT/LegacyNFT'
import * as status from '../order/status'
import * as addresses from '../../data/addresses'

export function isMint(event: Transfer): boolean {
  return event.params._from.toHexString() == addresses.Null
}
...
export function getTokenURI(event: Transfer): string {
  let erc721 = LegacyNFT.bind(event.address)
